...
{
    public abstract class RatingWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static RatingWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(RatingWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<RatingDbContext> action)
        {
            using (var context = IocManager.Resolve<RatingDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<RatingDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<RatingDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<RatingDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<RatingDbContext, Task<T>> func)
        {
            T result;
